...
def create_vocab(captions_dic):
  vocab = set(['unk'])
  for caption in captions_dic.values():
...
def get_indexed_caption(caption, word2idx):
    indexed_caption = []
    for w in caption.split():
        idx = word2idx[w] if w in word2idx else word2idx['unk']
    return " ".join(indexed_caption)
        


def load_mlm_data(args):
    train_path = os.path.join(args.data_dir,'train2014')
    val_path = os.path.join(args.data_dir,'val2014')
    test_path = os.path.join(args.data_dir,'test2014')
    ...
    train_image_ids = [int(name.split('.')[0].split('_')[-1]) for name in os.listdir(train_path)]
    val_image_ids = [int(name.split('.')[0].split('_')[-1]) for name in os.listdir(val_path)]
    ...
    train_captions_fname = os.path.join(args.captions_dir, "captions_train2014.json")
    val_captions_fname = os.path.join(args.captions_dir, "captions_val2014.json")
    train_captions = get_captions(train_captions_fname)
    val_captions = get_captions(val_captions_fname)
    
    vocab = set.union(create_vocab(train_captions), create_vocab(val_captions))
    
    
    # data: {id: caption}
    train_data = [(os.path.join(train_path, "COCO_train2014_{}.jpg".format(str(id).zfill(12))), train_captions[id]) for id in train_image_ids]
    val_data = [(os.path.join(val_path, "COCO_val2014_{}.jpg".format(str(id).zfill(12))), val_captions[id]) for id in val_image_ids]
    ...
    train_data = random.sample(train_data, int(args.train_pct * len(train_data)))
    val_data = random.sample(val_data, int(args.valid_pct * len(val_data)))
...
def test(loader, model, device, scaler, idx2word):
    ...
    with torch.no_grad():
        for (img,caption_token, segment_ids, attention_mask,target) in bar:
    ...
    total_acc = (PREDS == TARGETS).mean() * 100.
    bleu = calculate_bleu_score(PREDS,TARGETS, idx2word)

    return PREDS, total_acc, bleu



class ROCO(Dataset):
    def __init__(self, args, data, tfm, keys, mode):
        self.data = data
        self.args = args
...
def calculate_bleu_score(preds,targets, idx2word):
  bleu_per_answer = np.asarray([sentence_bleu([idx2word[target].split()],idx2word[pred].split()) for pred,target in zip(preds,targets)])
  return np.mean(bleu_per_answer)
